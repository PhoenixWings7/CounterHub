...
import com.kadamus.counterhub.presentation.settings.SettingsActivity
...
fun AppBarActions(onAddNewCounter: () -> Unit) {
    ...
    val isDropdownShown = remember { mutableStateOf(false) }
    IconButton(onClick = { isDropdownShown.value = isDropdownShown.value.not() }) {
        Icon(
            imageVector = Icons.Outlined.MoreVert,
            contentDescription = stringResource(id = R.string.cd_expandable_menu)
        )
    }
    if (isDropdownShown.value) {
        val context = LocalContext.current
        DropdownMenu(
            expanded = isDropdownShown.value,
            onDismissRequest = { isDropdownShown.value = false })
        {
            DropdownMenuItem(
                text = { Text(text = stringResource(id = R.string.lbl_settings)) },
                leadingIcon = {
                    Icon(
                        Icons.Outlined.Settings,
                        stringResource(id = R.string.cd_settings)
                    )
                }, onClick = {
